...
const FullscreenModelPage = ({ pathname }) => {
  ...
  return (
    <a.div
      style={styles}
      className={`FullscreenModelPage overflow-hidden ${showFullscreenMode ? 'pointer-events-auto z-50' : 'pointer-events-none z-[-1]'}`}
    >
      ...
      <Canvas gl={{ pixelRatio: Math.min(window.devicePixelRatio, 2), antialias: true }}>
        <PerspectiveCamera makeDefault position={[0, 0.2, 12]} fov={70} near={0.1} far={70} />
